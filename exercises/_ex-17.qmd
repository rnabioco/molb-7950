...
cpp11bigwig provides `read_bigwig()`, and TxDb.Scerevisiae.UCSC.sacCer3.sgdGene provides gene annotations for the S. cerevisiae genome.
...
plot_mnase_acf <- plot_acf(??, title = "MNase ACF")
...
Next, import the bigwig file containing yeast nucleosome-sized fragments (via MNase-seq) using `cpp11bigwig::read_bigwig()`.
...
1.  Load each bigWig as a GRanges object with `cpp11bigwig::read_bigwig()`
...
big_wig <- read_bigwig(here("data/block-dna", file_name))
