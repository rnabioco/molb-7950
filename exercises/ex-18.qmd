...
This is an important step that will eventually impact our conclusions.
...
Once we have the values from `bed_map()`, we can group by the `win_coord` and
calculate a summary statistic for each window.

Remember that the window positions are the same relative to TSSs, so these
numbers represent a composite signal across all TSS.
...
First, we need to reformat our data slightly. Take a look at `acc_tbl` and think about how you might reorganize the following way:
...
#| eval: false
mnase_tbl <- filter(acc_tbl, sample_type == "MNase_Long") |>
  select(-big_wig) |>
  unnest(cols = c(tss_win_sum))
```
