...
Here, we model read coverage using the Poisson distribution. Given some genome size $G$ and and a number of reads collected $N$, we can approximate $\lambda$ from $N/G$, scaled by the size of the sequence read (about 100 bp).
...
genome_lambda <- total_reads / genome_size / read_len
...
ggplot(
  ...
  # convert p-vales to positive values for plotting
  aes(start, ___)
) + 
  geom_line() +
