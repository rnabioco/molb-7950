...
# New packages for sc-RNA-seq analysis
library(Seurat)
...
The homework for today will involve continuing to process the AML dataset that we used in the first assignment. 
...
![](../img/block-rna/standard_workflow_annotated_v2.png)
...
The `NormalizeData` function puts the normalized values into a new slot called "data". The original counts are in a slot called "counts"

![](../img/block-rna/Seurat_structure_2.png)

**Question 1**

Can you visualize the first 5 rows and columns of both the raw data and the normalized data?

*Hint: use the function `GetAssayData`, remember you can learn about the arguments for a function using `?GetAssayData`*

*Hint 2: the `GetAssayData` function will return a large matrix, be sure to subset this matrix to only the first 10 rows and columns*

```{r}
# visualize the first 5 rows and colums of the raw and normalized data
...
Discussion: Is it easier to interpret the raw or normalized counts?
...
*Question 3*
Can you find the top 3000 variable genes using `FindVariableFeatures`?

```{r}
# TODO find 3000 variable genes
FindVariableFeatures(so, nfeatures = 3000)
```

# Scaling and centering

Next, we will scale the normalized expression values. The scaled values will only be used for dimensionality reduction and clustering and not differential expression. The purpose of scaling is to set the expression values for each gene onto a similar numeric scale. This avoids issues of having a gene that is highly expressed being given more weight in the clustering simply because it has larger numbers. Scaling converts the normalized data into z-scores by default and the values are stored in the `scale.data` slot.
...
```{r}
#| eval: false
# for large data sets, just scale the variable genes:
so <- ScaleData(
  object = so,
...
**Question 4**
Without this modified PCA, how many PCs would be computed?
...
pl <- purrr::map(pcs, ~ PCAPlot(so, dims = .x))
...
**Question 5**
Make a UMAP plot colored by CD8A and CD19 expression like our PCA above
```{r}
# TODO UMAP colored by CD8A and CD19
FeaturePlot(so,
  c("CD8A", "CD19"),
  reduction = "umap",
  ncol = 2
...
**Question 6**
What clusters are the genes S100A8, CD79A and CD3G most highly expressed in?

*Hint try using `VlnPlot` like we used above. By default the violin plots will be separated by the active ident. If you want to learn more about plotting by specific aspects of the meta data, look at `?VlnPlot`*

```{r}
# TODO make violin plots of S100A8, CD79A, and CD3G
VlnPlot(so, features = c("S100A8", "CD79A", "CD3G"))
```
...
plot_grid(plotlist = pl, nrow = 1, ncol = 3)
```
...
**Question 7**
What does the padj column mean in this output? What are we correcting for? Why even with this correction do we say that these p-values are inflated?
...
top_markers <- markers |>
  ...
  # filter for positive fold change and expressed in at least 10% of cells
  filter(logFC > 0, padj < 0.05, pct_in > 10) |> 
  group_by(group) |>
  # rank by adjusted p-value then logFC
  arrange(padj, desc(logFC), .by_group = TRUE) |>
  # show top genes per group
  slice_head()
...
1) In the early days of single cell sequencing, this was often done manually, by visual inspection of key genes (hopefully) using expertise in the lab.  
2) As more references become availible, it is more common to compare your data to other single cell data (see [Seurat Integration Vignette](https://satijalab.org/seurat/v3.2/integration.html) if interested) 
3) Compare your data to other cell type signature data (microarray, bulk rna-seq, marker gene lists). We use [`clustifyr`](https://rnabioco.github.io/clustifyr/) which was developed by a previous RBI fellow Rui Fu. There are many other methods but `clustifyr` is fast and it works.  
...
res <- clustify(
  ...
  # don't add to seurat object, just return results
  obj_out = FALSE
)
