...
```{r}
#| fig.alt: "Scatter plot of mean expression versus total variance for all genes, with the fitted mean-variance trend line and the ten most variable genes labeled."
...
`, or `
rank
` AUC as a potential ranking, which all will select for marker genes with different patterns:
...
Here we will plot top genes ranked by `
mean.AUC
` for cluster 1. 

```{r}
cluster1_markers <- as.data.frame(mrks[[1]]) |>
