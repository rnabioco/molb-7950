...
For the next two classes we'll be combining everything we've learned to process and visualize data from some biological experiments. These exercises will illustrate a complete analysis pipeline, from data tidying to manipulation and visualization, using tools from the tidyverse.
...
- After specific time points, cells were harvested and mRNAs of actin and interferon mRNA
were analyzed by quantitative PCR (with 3 technical replicates), with a control
containing no reverse transcriptase.
...
Note the structure of the tidied data. What columns are shared by both tibbles?

How we can *join* the data from these two tibble together, linking the sample identifiers with their gene expression values?
...
2. Calculate a ratio of interferon to actin levels for each sample *before*
   calculating the mead and standard deviation of the ratios.
...
Now we can plot the summary statistics. We'll use `geom_pointrange()` to represent the mean and standard deviation.
...
ggplot(
  ...
  geom_pointrange(
    aes(
      ymin = exp_mean - exp_sd,
      ymax = exp_mean + exp_sd
    ),
  ...
  labs(
    x = "Time (hours)",
    y = "Expression level",
    title = "qPCR analysis in wild-type and mutant cells"
  )
```
...
The experiment uses [chemostats](https://dunham.gs.washington.edu/chemostatv2.htm) to control the growth rate of cells under different nutrient-limited conditions.

In this experiment, cells were grown in different media limited for (one of) glucose, uracil, leucine, sulfate, phosphate, or ammonia. Over a series of fixed, equilibrium growth rates, cells were harvested and gene expression was measured by genome-wide microarrays.

### Load the data

A tidied version of the gene expression data are in:

-   `data/brauer_gene_exp.tsv.gz`
...
```{r}
brauer_gene_exp <- read_tsv(here("data/brauer_gene_exp.tsv.gz"))
...
brauer_mat_dat <-
  ...
  pivot_wider(names_from = sample, values_from = exp_level)
...
```{r}
filter(brauer_tidy, grepl("LEU", common_name)) |>
  ggplot(
    aes(
      x = rate,
      y = exp_level,
...
### Modeling gene expression to identify other significant gene-nutrient relationships
...
linear_models <-
  brauer_gene_exp |>
  drop_na() |>
  nest(.by = c(systematic_name, nutrient)) |>
  mutate(
    model = purrr::map(data, ~ lm(exp_level ~ rate, data = .x))
  ) 
