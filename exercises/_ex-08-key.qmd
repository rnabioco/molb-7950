...
```{r}
brauer_gene_exp_wide <- read_tsv(here("data/bootcamp/brauer_gene_exp_wide.tsv.gz"))
yeast_go_terms <- read_tsv(here("data/bootcamp/yeast_go_terms.tsv.gz"))
```
...
```{r}
#| fig.alt: "Scatter plots of expression versus growth rate for LEU genes, faceted by gene, with points colored by limiting nutrient and a linear fit per nutrient."
left_join(brauer_gene_exp_tidy, yeast_go_terms) |>
  ...
  theme_cowplot() +
  scale_color_manual(
    values = unname(palette.colors(palette = "Okabe-Ito"))
  )
```
