...
gene_name_map <-
  dplyr::select(t2g, -ensembl_transcript_id) |>
  unique()
```
...
salm_dirs <- fs::path(
   here('data/block-rna/salmonouts/'),
   sample_ids,
   "quant.sf.gz"
)
...
We can see here that this is a data frame where the rows are genes and the columns are interesting data.
...
differentiation.results.lfc.sig <-
  mutate(
    differentiation.results.lfc,
    sig = case_when(
      padj < 0.01 ~ 'yes',
...
```{r}
tpms <- pivot_longer(tpms, -c(ensembl_gene_id, gene))
head(tpms)
```
...
```{r}
tpms <- mutate(tpms, condition = ifelse(grepl('DIV0', name), 'DIV0', 'DIV7'))
head(tpms)
```
...
differentiation.results.annot$annot <-
  factor(
    differentiation.results.annot$annot, 
...
p.pluripotency <- tidy(
  wilcox.test(
    filter(differentiation.results.annot, annot == 'pluripotency')$log2FoldChange,
...
p.cellcycle <- tidy(
  wilcox.test(
    filter(differentiation.results.annot, annot == 'cellcycle')$log2FoldChange,
    filter(differentiation.results.annot, annot == 'none')$log2FoldChange
  )
)
...
p.neurondiff <- tidy(
  wilcox.test(
    filter(differentiation.results.annot, annot == 'neurondiff')$log2FoldChange,
    filter(differentiation.results.annot, annot == 'none')$log2FoldChange)
)
...
ggplot(
  ...
  labs(
    x = "Gene class",
    y = "DIV7 expression / DIV0 expression, log2"
  ) +
  geom_hline(
    yintercept = 0,
    color = 'gray',
