...
Your submission (an Rmarkdown file) is due Tues Sept 5 by 12pm. If you submit an entry, you'll get full credit on the problem set. Entries will be anonymized and winners will selected by popular vote.
...
```{r ok, message=FALSE, warning=FALSE}
library(tidyverse)
...
ggplot(penguins_clean,
       aes(x = body_mass_g / 1000, y = bill_length_mm)) +
  geom_point(aes(shape = sex, color = species)) +
  facet_grid(~ island) +
  theme_minimal_grid() +
...
ggplot(
  titanic_tbl,
  aes(x = Survived, y = n, color = Class, shape = Sex, size = 6)
) +
  geom_jitter() +
  scale_y_sqrt() +
  labs(
