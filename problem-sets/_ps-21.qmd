...
#| label: plot-peak-sig
#| eval: false
#| fig.alt: "Line plot of peak significance across a region of chromosome II with a horizontal threshold line."
ggplot(
...
Note: MEME (the underlying motif discovery software) was run ahead of time on the peak
sequences below, since it isn't installed in this environment. The output is provided for
you to import with `memes`.

Use Google and Pubmed to identify a study that defines a Reb1 motif using a genomewide analysis. a) How well does your 
motify match the previously defined one? b) Provide citation/link for the published motif(s) that you found.
...
# import the pre-computed MEME output provided for this problem set
meme_motifs <- universalmotif::read_meme(
  here("data/block-dna/reb1_meme_out/meme.txt")
)
...
# to look at the consensus motifs
#meme_df$consensus
...
#| label: plot-logo
#| eval: false
# meme_results is sorted by significance, so the first row is the top hit
