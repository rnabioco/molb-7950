...
M_avg <- biochem_WvG %>% filter(gender=="M") %>% summarise(avg_weight=mean(weight)) %>%  pull(avg_weight)

F_avg <- biochem_WvG %>% filter(gender=="F") %>% summarise(avg_weight=mean(weight)) %>%  pull(avg_weight)
...
```{r, echo=F}
tibble("Parameter" = c("$p_{mean}$","$p_{fit}$"),
       "fit cholesterol" = c("?","?"),
       "fit gender" = c("?","?"),
       "fit family" = c("?","?")
       )  %>% 
