...
```{r}
#| label: load-data
qpcr_names <- read_tsv(here("data/bootcamp/qpcr_names_ps.tsv.gz"))
qpcr_data <- read_tsv(here("data/bootcamp/qpcr_data_ps.tsv.gz"))
```
...
```{r}
#| fig.alt: "Point-range plot of mean qPCR expression over time with standard deviation error bars, colored by gene and faceted by genotype."
ggplot(
  qpcr_summary,
  aes(
    x = time,
...
```{r}
#| fig.alt: "Scatter plot of the IFN-beta to GAPDH expression ratio over time, faceted by genotype."
qpcr_summary |>
  select(-bio_sd) |>
