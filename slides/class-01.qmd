...
- `==` tests for equivalence (e.g. `x == 1` says "does `x` equal 1?")
...
- data.frame columns can have different types (numeric, character, factor, etc.). 

- A data.frame is a rectangle, where each column is a vector, and each row is a slice across vectors.

- A data.frame can be constructed by `data.frame()`.
