...
#| fig-height: 9
#| echo: true
#| fig.alt: "Scatter plot of total variance against mean expression for all genes, with the ten most variable genes labelled and the fitted mean-variance trend drawn as a dashed blue line."
...
#| fig-height: 9
#| echo: true
#| fig.alt: "Scatter plot of total variance against mean expression with genes colored by whether they were selected as highly variable, and the mean-variance trend as a dashed blue line."
...
#| echo: false
#| message: false
#| fig.alt: "Side-by-side panels showing two correlated simulated genes before and after PCA; the original scatter is diagonal, and the rotated PCA scatter spreads the variance along PC1, with the same points labelled in each."
library(MASS)
library(ggrepel)
library(dplyr)
...
sim <- MASS::mvrnorm(
  ...
  arrange(V1, V2) %>%
  dplyr::rename(gene_1 = V1, gene_2 = V2) %>%
  as_tibble() %>%
  mutate(id = row_number())

# select points to label for visualization
max_min_points <- dplyr::slice(sim, c(1:5, 95:100))
...
plot_labeled_pca <- function(input_data, pca_obj) {
  # tidy the pc matrix
  pc_x <- pca_obj$x %>%
    as.data.frame() %>%
  ...
  max_min_points <- dplyr::slice(pc_x, c(1:5, 95:100))
  # compute variance explained
  percent_var <- (100 * pca_obj$sdev^2 / sum(pca_obj$sdev^2)) %>%
    signif(3)
  # plot the data
...
```{r}
#| echo: false
#| fig.alt: "Side-by-side PCA plots comparing two correlated dimensions against the same two dimensions plus ten noise dimensions, showing how added noise reduces the variance explained by PC1."
