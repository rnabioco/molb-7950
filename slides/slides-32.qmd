...
A large source of variation arises due to differences in sequencing depth between cells. See PCA perform on unnormalized counts on right ->  
...
Using fewer dimensions makes computation much quicker and as we will see will reorder the data in a manner that captures the heterogeneity in the data
...
plot_labeled_pca <- function(input_data, 
  ...
  pca_plt <- ggplot(pc_x, aes(PC1, PC2)) +
    ...
    geom_text_repel(data = max_min_points, 
                  aes(label = id)) +
    labs(x = paste("PC1 ", percent_var[1], "%"),
...
more_genes <- matrix(rnorm(1000, 
                     ...
                     ncol = 10)
# add column names
colnames(more_genes) <- paste0("gene_noise_",
                              1:ncol(more_genes))
