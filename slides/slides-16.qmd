...
You can use `read_bed()` and related `read_*` functions from valr to load genome annotations and signals.
...
::: {.callout-note}
Later we'll also use the valr functions `read_bedgraph()` and `read_bigwig()`.
:::
