...
# the t2g table below is provided directly, so the live biomaRt query is not
# run at render time -- it is shown here for reference only
...
## Prepare `metadata` and import {.smaller}
...
#| fig.alt: "Volcano plot of log2 fold change versus negative log10 FDR for DIV7 relative to DIV0, with genes below 1% FDR colored red."
...
#| echo: true
#| output-location: slide
#| fig.alt: "Volcano plot of shrunken log2 fold change versus negative log10 FDR for DIV7 relative to DIV0, with genes below 1% FDR colored red."
ggplot(
  diff_lfc_sig,
  aes(
    x = log2FoldChange,
...
#| label: plot-tpms
#| output-location: column-fragment
#| fig.alt: "Jittered points of TPM values by condition, faceted by gene, with conditions colored blue and red."
...
pathwaygenes <- read.table(here('data', 'block-rna', 'pathwaygenes.csv.gz'), header = T, sep = ',')
pluripotencygenes <- filter(pathwaygenes, path == 'pluri')
cellcyclegenes <- filter(pathwaygenes, path == 'cellcycle')
neurongenes <- filter(pathwaygenes, path == 'neuron')
```
...
#| warning: false
#| output-location: slide
#| fig.alt: "Notched box plots of log2 fold change (DIV7 over DIV0) by gene class, with significance bars and p-values annotated above the boxes."
ggplot(
  diff_paths,
  aes(
    x = annot,
