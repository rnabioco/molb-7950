...
``` bash
less data/block-rna/salmonouts/DIV0.Rep1/logs/salmon_quant.log
```
...
grep 'Mapping ' data/block-rna/salmonouts/*/logs/salmon_quant.log
```
...
metadata <- data.frame(sample_id = list.files(here("data/block-rna/salmonouts"), pattern = "^DIV"),
                   salmon_dirs = list.files(here("data/block-rna/salmonouts"),recursive = T,pattern = ".gz$", full.names = T)
